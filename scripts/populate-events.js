...
if (!supabaseUrl || !supabaseKey) {
  ...
  console.log('Make sure .env.local exists in the root directory with:');
...
function createSlug(title) {
  if (!title || typeof title !== 'string') {
    console.warn('Warning: Invalid title provided to createSlug:', title);
...
function parseDate(dateStr) {
  ...
  // Handle DDMMYY format (e.g., 161225 = 16/12/25)
...
function categorizeEvent(title, description) {
  ...
  if (titleLower.includes('tech') || titleLower.includes('hackathon') || descLower.includes('innovation')) return 'Technology';
  if (titleLower.includes('market') || titleLower.includes('food') || titleLower.includes('art')) return 'Marketplace';
  if (titleLower.includes('culture') || titleLower.includes('traditional') || titleLower.includes('heritage')) return 'Cultural';
  if (titleLower.includes('miss') || titleLower.includes('pageant')) return 'Pageant';
  if (titleLower.includes('sport') || titleLower.includes('football') || titleLower.includes('competition')) return 'Sports';
  if (titleLower.includes('business') || titleLower.includes('investment') || titleLower.includes('forum')) return 'Business';
  if (titleLower.includes('motor') || titleLower.includes('race') || titleLower.includes('drift')) return 'Automotive';
  if (titleLower.includes('camp') || titleLower.includes('hiking') || titleLower.includes('fishing')) return 'Adventure';
  if (titleLower.includes('festival') || titleLower.includes('tea') || titleLower.includes('suya')) return 'Festival';
  if (titleLower.includes('minister') || titleLower.includes('first lady')) return 'Official';
  
  return 'General';
}

// Helper function to extract price
function extractPrice(registrationStr) {
  if (!registrationStr || registrationStr === '') return null;
  
  // Remove commas and extract numbers
  const numbers = registrationStr.replace(/,/g, '').match(/\d+/g);
...
async function populateEvents() {
  try {
    ...
    const { error: deleteError } = await supabase
      .from('events')
      .delete()
      .neq('id', '00000000-0000-0000-0000-000000000000'); // Delete all existing events
    ...
    const eventsWithSlugs = events.map((event, index) => {
      if (!event.title) {
        console.warn(`Warning: Event at index ${index} has no title:`, event);
        return null; // Skip this event
      }
      // Create clean event object with only database columns
      const cleanEvent = {
        title: event.title,
      ...
      if (event.start_date) cleanEvent.start_date = event.start_date;
      if (event.end_date) cleanEvent.end_date = event.end_date;
      if (event.registration_fee) cleanEvent.registration_fee = event.registration_fee;
      if (event.featured !== undefined) cleanEvent.featured = event.featured;

      return cleanEvent;
    }).filter(event => event !== null); // Remove null events

    console.log(`📊 Total events to process: ${events.length}`);
    console.log(`✅ Valid events after filtering: ${eventsWithSlugs.length}`);
    
    if (eventsWithSlugs.length === 0) {
      console.error('❌ No valid events to insert!');
      return;
    }

    console.log(`Attempting to insert ${eventsWithSlugs.length} events...`);
